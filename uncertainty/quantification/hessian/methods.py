...
def bfgs(x, func):
    ...
    func_value = lambda x: func(torch.tensor(x, requires_grad=True))
    grad_func = lambda x: torch.autograd.grad(func_value(x_tensor), x_tensor, retain_graph=True, allow_unused=True)[0]
    grad = grad_func(x).detach().numpy() if grad_func(x) is not None else None

...
def bfgs(x, func):
    ...
    grad = lambda x: torch.autograd.grad(func_value(x_tensor), x_tensor, retain_graph=True, allow_unused=True)[0]
    grad = grad.detach().numpy() if grad is not None else None
